...
<body>
        ...
        <table border="1">
            ...
            <%
                try (PreparedStatement stmt = con.prepareStatement(
                        "SELECT ts.TrainStopID, s.Name, ts.ArrivalTime, ts.DepartureTime " +
                        "FROM TrainStops ts " +
            ...
            %>
            <tr>
                <td><%= rs.getInt("TrainStopID") %></td>
                <td><%= rs.getString("Name") %></td>
                <td>
                    <input type="datetime-local" name="arrivalTime_<%= rs.getInt("TrainStopID") %>" 
                           value="<%= rs.getTimestamp("ArrivalTime").toString().replace(" ", "T") %>">
                </td>
                <td>
                    <input type="datetime-local" name="departureTime_<%= rs.getInt("TrainStopID") %>" 
                           value="<%= rs.getTimestamp("DepartureTime").toString().replace(" ", "T") %>">
                </td>
                <td>
                    <form action="deleteSchedule.jsp" method="POST">
                        <input type="hidden" name="TrainStopID" value="<%= rs.getInt("TrainStopID") %>">
    ...
    <form method="POST" action="addStopSchedule.jsp">
        <input type="hidden" name="scheduleID" value="<%= scheduleID %>">
        <label for="station">Station:</label>
        <select name="stationID">
            <%
                try (Statement stmt = con.createStatement()) {
                    ResultSet rs = stmt.executeQuery("SELECT StationID, Name FROM Stations");
    ...
    <br>
    <a href="customerRep.jsp">Back to Dashboard</a>
</body>
</html>
